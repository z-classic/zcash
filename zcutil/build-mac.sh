...
$0 [ --enable-lcov || --disable-tests ] [ --disable-mining ] [ --disable-rust ] [ --enable-proton ] [ MAKEARGS... ]
  Build Joshcoin and most of its transitive dependencies from
  source. MAKEARGS are applied to both dependencies and Joshcoin itself.

  If --enable-lcov is passed, Joshcoin is configured to add coverage
  instrumentation, thus enabling "make cov" to work.
  If --disable-tests is passed instead, the Joshcoin tests are not built.

  If --disable-mining is passed, Joshcoin is configured to not build any mining
  code. It must be passed after the test arguments, if present.

  If --disable-rust is passed, Joshcoin is configured to not build any Rust language
  assets. It must be passed after test/mining arguments, if present.

  If --enable-proton is passed, Joshcoin is configured to build the Apache Qpid Proton
