...
if [ ! -f "$HOME/Library/Application Support/Joshcoin/joshcoin.conf" ]; then
    echo "Creating joshcoin.conf"
    mkdir -p "$HOME/Library/Application Support/Joshcoin/"
    echo "rpcuser=zcashrpc" > ~/Library/Application\ Support/Joshcoin/joshcoin.conf
    PASSWORD=$(cat /dev/urandom | env LC_CTYPE=C tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
    echo "rpcpassword=$PASSWORD" >> "$HOME/Library/Application Support/Joshcoin/joshcoin.conf"
    echo "Complete!"
fi
