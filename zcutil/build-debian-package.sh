...
# Copy binaries
cp $SRC_PATH/src/ZCLd $DEB_BIN
cp $SRC_PATH/src/ZCLi $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/zcashd.1 $DEB_MAN				// check!!!
cp $SRC_DOC/man/zcash-cli.1 $DEB_MAN			// check!!
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN	// check!
# Copy bash completion files
cp $SRC_PATH/contrib/zcashd.bash-completion $DEB_CMP/ZCLd
cp $SRC_PATH/contrib/zcash-cli.bash-completion $DEB_CMP/ZCLi
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/ZCLd.1
gzip --best -n $DEB_MAN/ZCLi.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/ZCLd $DEB_BIN/ZCLi
